...
class SwiGluMLP(nn.Module):
    ...
    def forward(self, x):
        if self.config.pretraining_tp > 1:
            ...
            print("Hossam: MLP x shape: ", x.shape, self.gate_proj.weight.shape)
    ...
    def scan_fn(carry, i_start, i_end):
        for i in range(i_start, i_end):
            if isinstance(xs, dict):
                ...
                x = [x[i] for x in xs]
            carry, y = f(carry, x)
            # print("Hossam carry y x: ", carry, y.shape)
            print("Hossam carry W1_states shape: ", carry['W1_states'].shape)
            out[i] = y
        return carry
...
class TTTBase(nn.Module):
    ...
    def get_qkv_projections(self, hidden_states, cache_params: Optional[TTTCache] = None):
        if self.share_qk:
            ...
            print("Hossam self.q_proj ", self.q_proj.weight.shape, hidden_states.shape, XQ.shape)
        return XQ, XK, XV
    ...
    def get_ttt_inputs(self, inputs, mini_batch_size, cache_params):
        ...
        print("Hossam inputs to ttt: ", X.shape, XQ.shape)
        if cache_params is not None:
    ...
    def forward(
        ...
        print("Hossam Input to linear projections: ", hidden_states.shape)
        XQ, XK, XV = self.get_qkv_projections(hidden_states, cache_params=cache_params)

        print("Hossam: input xq, xk, xv linear projections: ", XQ.shape, " num_mini_batch= ", num_mini_batch, " L= ", L, " mini_batch_size= ", self.mini_batch_size)
        ...
        print("Hossam output hidden states: o_proj: ", output_hidden_states.shape)
...
class TTTLinear(TTTBase):
    ...
    def ttt(
        ...
        print(f"Hossam B={B} num_mini_batch={num_mini_batch} L={L}")
        # NOTE:
        ...
        def compute_mini_batch(params_dict, inputs):
            ...
            # print("Hossam W1_init looks: ", W1_init)
            ...
            print("[X1 @ W1_init] Hossam Operation 1: o=[B,nh,K,f]", X1.shape, getProd(X1.shape), W1_init.shape, getProd(W1_init.shape), Z1.shape, getProd(Z1.shape), use_dual_form)
            ...
            # [B,nh,K,f] "Batch backward for LayerNorm fused with L2 loss."
            ...
            if use_dual_form:
                ...
                print("Hossam dual Attn1 = torch.tril(XQ_mini_batch @ X1.transpose(-2, -1)) ", XQ_mini_batch.shape, getProd(XQ_mini_batch.shape), X1.transpose(-2, -1).shape, getProd(X1.transpose(-2, -1).shape), Attn1.shape, getProd(Attn1.shape))


                # [B,nh,1,f] - [B,nh,K,K] @ [B,nh,K,f] -> [B,nh,K,f]
                b1_bar = b1_init - torch.tril(eta_mini_batch) @ grad_l_wrt_Z1

                print("Hossam dual b1_bar = b1_init - torch.tril(eta_mini_batch) @ grad_l_wrt_Z1  ", torch.tril(eta_mini_batch).shape, getProd(torch.tril(eta_mini_batch).shape), grad_l_wrt_Z1.shape, getProd(grad_l_wrt_Z1.shape),  b1_bar.shape, getProd(b1_bar.shape))

                # Hossam
                # Z1 = W0x - 2 eta delta * gradient + bias
                # (W0X−X)⊙mask⋅XT X => (eta_mini_batch * Attn1) @ grad_l_wrt_Z1
                # [B,nh,K,f] @ [B,nh,f,f] - ([B,nh,K,1] * [B,nh,K,K]) @ [B,nh,K,f] + [B,nh,K,f]
                # [B, nh, K, f]
                Z1_bar = XQ_mini_batch @ W1_init - (eta_mini_batch * Attn1) @ grad_l_wrt_Z1 + b1_bar

                print("i--Hossam dual Z1_bar = XQ_mini_batch @ W1_init  ", XQ_mini_batch.shape, 
                      getProd(XQ_mini_batch.shape), W1_init.shape, getProd(W1_init.shape), Z1_bar.shape, getProd(Z1_bar.shape))
                print("ii--Hossam dual Z1_bar = (eta_mini_batch * Attn1) @ grad_l_wrt_Z1  ", (eta_mini_batch * Attn1).shape, 
                      getProd((eta_mini_batch * Attn1).shape), grad_l_wrt_Z1.shape, getProd(grad_l_wrt_Z1.shape), Z1_bar.shape, getProd(Z1_bar.shape))

                last_eta_mini_batch = eta_mini_batch[:, :, -1, :, None]
                # [B,nh,f,f] - [B,nh,f,K] @ [B,nh,K,f]
                W1_last = W1_init - (last_eta_mini_batch * X1).transpose(-1, -2) @ grad_l_wrt_Z1

                print("Hossam dual W1_last = W1_init - (last_eta_mini_batch * X1).transpose(-1, -2) @ grad_l_wrt_Z1  ", (last_eta_mini_batch * X1).transpose(-1, -2) .shape, 
                      getProd((last_eta_mini_batch * X1).transpose(-1, -2).shape), grad_l_wrt_Z1.shape, getProd(grad_l_wrt_Z1.shape), W1_last.shape, getProd(W1_last.shape))
                ...
                print("[torch.einsum(bhki,bhkj->bhkij, X1, grad_l_wrt_Z1)] Hossam Operation 2: ", X1.shape, grad_l_wrt_Z1.shape, grad_W1.shape)

                grad_W1 = torch.einsum("bhnk,bhkij->bhnij", torch.tril(ttt_lr_eta_mini_batch), grad_W1)
                print("Hossam Operation 3: ", torch.tril(ttt_lr_eta_mini_batch).shape, grad_W1.shape, grad_W1.shape)
                ...
                print("Hossam Operation 4: ", torch.tril(ttt_lr_eta_mini_batch).shape, grad_l_wrt_Z1.shape, grad_b1.shape)
                ...
                print("Hossam Operation 5: ", XQ_mini_batch.unsqueeze(3).shape, W1_bar.shape, Z1_bar.shape)
        ...
        print("Hossam return output ", XQW_batch.shape)
...
class Block(nn.Module):
    def __init__(self, config: TTTConfig, layer_idx: int):
        ...
        # linear
        # print("Hossam TTT Layer type: ", config.ttt_layer_type)
...
class TTTModel(TTTPreTrainedModel):
    ...
    def forward(
        ...
        for decoder_layer in self.layers:
            # Hossam Block class calling
            # print("Hossam Decoder layer: ", decoder_layer)
            print("---------------------")
            print(f"Hossam iDecoder_Layer={idecoder_layer}")
            idecoder_layer += 1
